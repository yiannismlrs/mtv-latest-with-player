...
/**
 * Embed.su extractor - Alternative source
 */
public class EmbedSuExtractor extends MultiSourceExtractor.StreamExtractor {
    ...
    public CompletableFuture<ExtractionResult> extractStreams(MultiSourceExtractor.MediaInfo mediaInfo) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                Log.d(TAG, "Extracting from 2Embed.to: " + mediaInfo.title);
    ...
    private List<MultiSourceExtractor.ExtractedStream> extractStreamsFromHtml(String html, String referer) {
        ...
        // Look for iframe sources that might contain streams
        ...
        while (iframeMatcher.find()) {
            ...
            // Try to extract streams from iframe
            try {
                String iframeHtml = fetchPage(iframeSrc);
                if (iframeHtml != null) {
                    List<MultiSourceExtractor.ExtractedStream> iframeStreams = extractDirectStreams(iframeHtml, iframeSrc);
                    streams.addAll(iframeStreams);
                }
            } catch (Exception e) {
                Log.w(TAG, "Failed to process iframe: " + e.getMessage());
            }
        }
        
        // Also try direct extraction from main page
        streams.addAll(extractDirectStreams(html, referer));
        
        return streams;
    }
    
    private List<MultiSourceExtractor.ExtractedStream> extractDirectStreams(String html, String referer) {
        List<MultiSourceExtractor.ExtractedStream> streams = new ArrayList<>();
